...
def validate_yaml_frontmatter(component_type, file_path, result):
...
def validate_component(root_dir, component_type, file_path, all_ids, result):
    ...
    # 2. YAML frontmatter
    frontmatter = validate_yaml_frontmatter(component_type, file_path, result)
    if frontmatter is None:
...
def print_results(result, strict=False):
...
Examples:
  ...
  %(prog)s --no-color         # Disable colored output
    ...
    for component_type in components_to_validate:
        ...
        for file_path in files:
            validate_component(root_dir, component_type, file_path, all_ids, result)
    
    # Print results
    print_results(result, strict=args.strict)
