...
@test "VALIDATE: Validation runs by default during installation" {
  ...
  export VIBESAFE_TEMPLATES_DIR="$ORIGINAL_DIR"
  export VIBESAFE_INSTALL_WHATS_NEXT=true
...
# ==============================================================================
# CIP-0012: AI Assistant Framework Independence Tests
# ==============================================================================
# Note: These tests use local templates (ORIGINAL_DIR) because CIP-0012
# added templates/prompts/ which isn't in the GitHub clone yet.

@test "CIP-0012: Cursor rules are generated by default" {
  # Run installation with default settings using local templates
  VIBESAFE_TEMPLATES_DIR="$ORIGINAL_DIR" bash "$INSTALL_SCRIPT"
  
  # Verify Cursor rules directory exists
  [ -d ".cursor/rules" ]
  
  # Verify core base prompts were generated
  [ -f ".cursor/rules/vibesafe_general.mdc" ]
  [ -f ".cursor/rules/backlog.mdc" ]
  [ -f ".cursor/rules/cip.mdc" ]
  
  # Verify at least 3 cursor rule files exist
  RULE_COUNT=$(ls -1 .cursor/rules/*.mdc 2>/dev/null | wc -l)
  [ "$RULE_COUNT" -ge 3 ]
  
  # Verify they contain expected content
  grep -q "Git Operations - Surgical Precision Required" ".cursor/rules/vibesafe_general.mdc"
  grep -q "Code Improvement Plans" ".cursor/rules/cip.mdc"
}

@test "CIP-0012: Cursor rules have proper YAML frontmatter" {
  VIBESAFE_TEMPLATES_DIR="$ORIGINAL_DIR" bash "$INSTALL_SCRIPT"
  
  # Check that Cursor rules have required frontmatter
  [ -f ".cursor/rules/vibesafe_general.mdc" ]
  grep -q "^---$" ".cursor/rules/vibesafe_general.mdc"
  grep -q "^description:" ".cursor/rules/vibesafe_general.mdc"
  grep -q "^globs:" ".cursor/rules/vibesafe_general.mdc"
  grep -q "^alwaysApply:" ".cursor/rules/vibesafe_general.mdc"
}

@test "CIP-0012: All platforms generated with VIBESAFE_PLATFORM=all" {
  # Run installation with all platforms
  VIBESAFE_PLATFORM=all VIBESAFE_TEMPLATES_DIR="$ORIGINAL_DIR" bash "$INSTALL_SCRIPT"
  ...
  # Verify Cursor has multiple files
  [ -f ".cursor/rules/vibesafe_general.mdc" ]
  [ -f ".cursor/rules/cip.mdc" ]
  
  # Verify other platforms are single combined files
  [ -f ".github/copilot-instructions.md" ]
  [ -f "CLAUDE.md" ]
  [ -f "AGENTS.md" ]
}

@test "CIP-0012: VIBESAFE_PLATFORM=cursor generates only Cursor files" {
  VIBESAFE_PLATFORM=cursor VIBESAFE_TEMPLATES_DIR="$ORIGINAL_DIR" bash "$INSTALL_SCRIPT"
  
  # Verify Cursor files exist
  [ -d ".cursor/rules" ]
  [ -f ".cursor/rules/vibesafe_general.mdc" ]
  
  # Verify other platforms were NOT generated
  [ ! -f ".github/copilot-instructions.md" ]
  [ ! -f "CLAUDE.md" ]
  [ ! -f "AGENTS.md" ]
}

@test "CIP-0012: VIBESAFE_PLATFORM=copilot generates only Copilot files" {
  VIBESAFE_PLATFORM=copilot VIBESAFE_TEMPLATES_DIR="$ORIGINAL_DIR" bash "$INSTALL_SCRIPT"
  
  # Verify Copilot file exists
  [ -f ".github/copilot-instructions.md" ]
  
  # Verify it has content
  grep -q "VibeSafe Project Guidelines" ".github/copilot-instructions.md"
  grep -q "Git Operations - Surgical Precision Required" ".github/copilot-instructions.md"
  
  # Verify other platforms were NOT generated (check for actual rule files, not just directory)
  CURSOR_RULE_COUNT=$(ls -1 .cursor/rules/*.mdc 2>/dev/null | wc -l)
  [ "$CURSOR_RULE_COUNT" -eq 0 ]
  [ ! -f "CLAUDE.md" ]
  [ ! -f "AGENTS.md" ]
}

@test "CIP-0012: VIBESAFE_PLATFORM=claude generates only Claude files" {
  VIBESAFE_PLATFORM=claude VIBESAFE_TEMPLATES_DIR="$ORIGINAL_DIR" bash "$INSTALL_SCRIPT"
  
  # Verify Claude file exists
  [ -f "CLAUDE.md" ]
  
  # Verify it has content
  grep -q "VibeSafe Project Memory" "CLAUDE.md"
  grep -q "Git Operations - Surgical Precision Required" "CLAUDE.md"
...
@test "CIP-0012: VIBESAFE_PLATFORM=codex generates only Codex files" {
  VIBESAFE_PLATFORM=codex VIBESAFE_TEMPLATES_DIR="$ORIGINAL_DIR" bash "$INSTALL_SCRIPT"
  
  # Verify Codex file exists
  [ -f "AGENTS.md" ]
  
  # Verify it has content
  grep -q "VibeSafe Project Documentation for" "AGENTS.md"
  grep -q "Git Operations - Surgical Precision Required" "AGENTS.md"
...
@test "CIP-0012: Content consistency across platforms" {
  VIBESAFE_PLATFORM=all VIBESAFE_TEMPLATES_DIR="$ORIGINAL_DIR" bash "$INSTALL_SCRIPT"
  
  # Verify all files contain core VibeSafe content
  grep -q "Git Operations - Surgical Precision Required" ".cursor/rules/vibesafe_general.mdc"
  grep -q "Git Operations - Surgical Precision Required" ".github/copilot-instructions.md"
  grep -q "Git Operations - Surgical Precision Required" "CLAUDE.md"
  grep -q "Git Operations - Surgical Precision Required" "AGENTS.md"
  
  # Verify CIP guidance is present in all platforms
  grep -q "Code Improvement Plans" ".cursor/rules/cip.mdc"
  grep -q "Code Improvement Plans" ".github/copilot-instructions.md"
  grep -q "Code Improvement Plans" "CLAUDE.md"
  grep -q "Code Improvement Plans" "AGENTS.md"
}

@test "CIP-0012: Platform files are non-empty and properly sized" {
  VIBESAFE_PLATFORM=all VIBESAFE_TEMPLATES_DIR="$ORIGINAL_DIR" bash "$INSTALL_SCRIPT"
  ...
  # Verify they're reasonably large (should be at least 10KB each)
  # Using wc -c for byte count
  COPILOT_SIZE=$(wc -c < ".github/copilot-instructions.md")
  CLAUDE_SIZE=$(wc -c < "CLAUDE.md")
  AGENTS_SIZE=$(wc -c < "AGENTS.md")
  
  [ "$COPILOT_SIZE" -gt 10000 ]
  [ "$CLAUDE_SIZE" -gt 10000 ]
  [ "$AGENTS_SIZE" -gt 10000 ]
}

@test "CIP-0012: Backward compatibility - default generates Cursor rules" {
  # Run without specifying platform (should default to 'all')
  VIBESAFE_TEMPLATES_DIR="$ORIGINAL_DIR" bash "$INSTALL_SCRIPT"
  
  # Verify Cursor rules still generated (backward compatibility)
  [ -d ".cursor/rules" ]
  [ -f ".cursor/rules/vibesafe_general.mdc" ]
  [ -f ".cursor/rules/cip.mdc" ]
  [ -f ".cursor/rules/backlog.mdc" ]
  
  # Should also generate other platforms by default
  [ -f ".github/copilot-instructions.md" ]
  [ -f "CLAUDE.md" ]
  [ -f "AGENTS.md" ]
}

@test "CIP-0012: Platform-specific directories in gitignore" {
  VIBESAFE_PLATFORM=all VIBESAFE_TEMPLATES_DIR="$ORIGINAL_DIR" bash "$INSTALL_SCRIPT"
  
  # Verify all platform directories are in gitignore
  [ -f ".gitignore" ]
  grep -q "^\.cursor/rules/$" ".gitignore"
  grep -q "^\.github/copilot-instructions\.md$" ".gitignore"
  grep -q "^CLAUDE\.md$" ".gitignore"
  grep -q "^AGENTS\.md$" ".gitignore"
}

@test "CIP-0012: Platform files are marked as system files in gitignore" {
  VIBESAFE_TEMPLATES_DIR="$ORIGINAL_DIR" bash "$INSTALL_SCRIPT"
  
  [ -f ".gitignore" ]
  
  # Verify gitignore has AI context section
  grep -q "# AI context files" ".gitignore" || \
  grep -q "# VibeSafe System Files" ".gitignore"
  
  # Verify platform files are listed
  grep -q "\.cursor/rules/" ".gitignore"
  grep -q "copilot-instructions\.md" ".gitignore"
  grep -q "CLAUDE\.md" ".gitignore"
  grep -q "AGENTS\.md" ".gitignore"
}

@test "CIP-0012: Copilot file has proper header" {
  VIBESAFE_PLATFORM=copilot VIBESAFE_TEMPLATES_DIR="$ORIGINAL_DIR" bash "$INSTALL_SCRIPT"
  
  # Verify Copilot file has appropriate header containing VibeSafe
  head -10 ".github/copilot-instructions.md" | grep -qi "VibeSafe"
}

@test "CIP-0012: Claude file has proper header" {
  VIBESAFE_PLATFORM=claude VIBESAFE_TEMPLATES_DIR="$ORIGINAL_DIR" bash "$INSTALL_SCRIPT"
  
  # Verify Claude file has appropriate header containing VibeSafe
  head -10 "CLAUDE.md" | grep -qi "VibeSafe"
}

@test "CIP-0012: Codex file has proper header" {
  VIBESAFE_PLATFORM=codex VIBESAFE_TEMPLATES_DIR="$ORIGINAL_DIR" bash "$INSTALL_SCRIPT"
  
  # Verify Codex file has appropriate header containing VibeSafe
  head -10 "AGENTS.md" | grep -qi "VibeSafe"
}

@test "CIP-0012: Invalid platform falls back to generic context" {
  # Test with invalid platform name
  run bash -c "VIBESAFE_PLATFORM=invalid VIBESAFE_TEMPLATES_DIR='$VIBESAFE_TEST_TEMPLATES' bash '$INSTALL_SCRIPT' 2>&1"
  
  # Should complete successfully with warning
  [ "$status" -eq 0 ]
  [[ "$output" == *"Warning"* ]] || [[ "$output" == *"Unknown platform"* ]]
}

@test "CIP-0012: Platform generation preserves existing user content" {
  # Create some user content first
  echo "# My Project" > README.md
  mkdir -p cip
  echo "# My CIP" > cip/cip0001.md
  ...
  [ -f "README.md" ]
  grep -q "My Project" "README.md"
  [ -f "cip/cip0001.md" ]
  grep -q "My CIP" "cip/cip0001.md"
  
  # Verify platform files were still generated
  [ -f ".github/copilot-instructions.md" ]
  [ -f "CLAUDE.md" ]
  [ -f "AGENTS.md" ]
}
