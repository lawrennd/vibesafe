...
created: "2026-01-04"
id: "0012"
last_updated: "2026-01-04"  # Closed 2026-01-04
status: "Closed"
compressed: true  # Compressed into README.md Framework Independence section (2026-01-08)
related_requirements: ["000C", "000D"]
related_cips: ["0010"]
tags:
- cip
...
- [x] In Progress - Actively being implemented (Phases 1-4)
- [x] Implemented - Work complete, all phases tested ✅
- [x] Closed - Complete with comprehensive test coverage (17 automated tests) 🎉
...
└── templates/prompts/ ← Platform-agnostic base content
    ...
    └── .cursorrules ← Generated for legacy Cursor
```

### Current VibeSafe Prompt Structure
...
- **Best match**: Combined file OR separate slash commands

**Codex CLI**:
- ✅ Project docs: `AGENTS.md` or `codex.md`
- ❌ No automatic path-specific: Applies to whole project
- **Best match**: Single combined file

### Prompt Composition Strategy: Hybrid Approach

**Recommendation**: Use each platform's capabilities fully (**Option 3: Hybrid**)

**Rationale**:
1. **Cursor** (70%+ of current users): Keep multi-file structure (best UX, already working)
2. **Copilot** (supports path-specific): Use `.github/instructions/*.instructions.md` (maintain same precision)
3. **Claude Code** (memory + commands): `CLAUDE.md` for always-apply + `.claude/commands/` for on-demand
4. **Codex** (single file only): `AGENTS.md` combined file (trade-off: lose path-specificity, but platform doesn't support it)
...
**Claude Code**: Combined memory + on-demand commands
```bash
...
1. **Base Prompts as Source of Truth**
   ...
   - Organize into `always-apply/` and `context-specific/` subdirectories
...
2. **Installation-Time Generation**
   ...
   - Auto-detect or accept platform flag: `VIBESAFE_PLATFORM=cursor|copilot|claude|codex|all`
   - Default to generating all common platforms for maximum compatibility
   - Each platform gets format optimized for its capabilities
...
4. **Platform Adapters**
   - Create adapter functions for each platform
   - Handle platform-specific formatting (YAML frontmatter variations, etc.)
   - Use platform capabilities fully (path-specific where supported)
...
- **REQ-000D**: CIPs as Self-Contained Design Documents (this CIP follows the requirement by integrating all design content)

Specifically, it implements solutions for:
- Multi-platform AI context discovery (`.cursor/rules/`, `.github/copilot-instructions.md`, `CLAUDE.md`, `AGENTS.md`)
...
- Self-contained design (all composition strategy integrated into this CIP)
