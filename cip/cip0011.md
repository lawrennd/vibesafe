...
related_requirements: ["0001", "0002", "0003", "0004", "0005", "0006", "0007"]
---
...
This CIP addresses HOW to achieve the outcomes defined in requirements 0001 through 0007:
...
### Phase 0a: Create Validation Script (1-2 hours) **[DO FIRST]**
...
- [ ] **YAML Frontmatter Validation**:
  - [ ] Parse all component files (requirements, CIPs, backlog, tenets)
  - [ ] Check required fields present for each component type
  - [ ] Validate field values (status in allowed list, dates formatted YYYY-MM-DD)
  - [ ] Report missing or invalid fields with file path and line number
- [ ] **File Naming Validation**:
  - [ ] Check requirements match `reqXXXX_short-name.md` pattern
  - [ ] Check CIPs match `cipXXXX.md` or `cipXXXX_short-name.md` pattern
  - [ ] Check backlog match `YYYY-MM-DD_short-name.md` pattern
  - [ ] Check tenets match `short-name.md` pattern (kebab-case)
- [ ] **Cross-Reference Validation** (basic):
  - [ ] Check requirements reference valid tenet IDs
  - [ ] Check CIPs reference valid requirement IDs
  - [ ] Check backlog references valid CIP IDs
  - [ ] Warn on broken references with suggestion to fix
- [ ] **Bottom-Up Pattern Validation**:
  - [ ] Warn if requirements have `related_cips` or `related_backlog` (should only have related_tenets)
  - [ ] Warn if CIPs have `related_backlog` (should only have related_requirements, related_cips)
  - [ ] Warn if backlog has `related_requirements` (should only have related_cips)
  - [ ] Warn if tenets have ANY related_ fields (foundation layer)
- [ ] **Output Format**:
  - [ ] Color-coded: ✅ success, ⚠️  warnings, ❌ errors
  - [ ] Group by validation type (YAML, naming, cross-refs, pattern)
  - [ ] Show file paths and specific issues
  - [ ] Exit code 0 if no errors (warnings OK), 1 if errors
- [ ] **Command-line Options**:
  - [ ] `--strict` mode (warnings become errors)
  - [ ] `--component [req|cip|backlog|tenet]` (validate only specific type)
  - [ ] `--fix` mode (auto-fix simple issues like date formatting)
- [ ] Test script on VibeSafe itself (should find existing issues!)
- [ ] Document usage in README

**Deliverable**: `scripts/validate_vibesafe.py` - our gold standard conformance checker

### Phase 0: YAML Frontmatter and Naming Standardization (1-2 hours)

- [ ] **Standardize file naming conventions**:
  - [ ] Document naming patterns for all component types
  - [ ] Requirements: `reqXXXX_short-name.md` (4-digit decimal, lowercase)
  - [ ] CIPs: `cipXXXX_short-name.md` (4-digit hex, lowercase, short name optional)
  - [ ] Backlog: `YYYY-MM-DD_short-name.md` (already consistent)
  - [ ] Tenets: `short-name.md` (already consistent)
  - [ ] Note: Existing CIPs without short names don't need renaming (optional feature)
...
- [ ] **Update backlog template** (enforce bottom-up pattern):
  - [ ] Ensure `related_cips` field exists
  - [ ] Remove `related_requirements` (violates bottom-up pattern - query through CIPs)
  - [ ] Add note: "Backlog tasks are DOING the work defined in CIPs"
- [ ] Add YAML validation to parsing scripts
- [ ] Document YAML schema and file naming for each component type
...
- [ ] **Update install-minimal.sh to handle migration (REQ-007)**:
