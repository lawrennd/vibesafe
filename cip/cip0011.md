...
- Traceability: Tenet → Requirements → CIPs → Backlog
...
├── integrations/ (backlog, cip, common)
├── examples/ (4 examples)
├── guidance/ (conversation tips)
└── [Actual requirement files scattered around]

Proposed Structure (SIMPLE):
requirements/
├── README.md (simple usage guide)
├── req0001_yaml-standardization.md
├── req0002_simplify-requirements-framework.md
└── archive/ (completed/rejected requirements)
```
...
**Decision Rules:**
- **Simple requirement** → Create a requirement file in `requirements/`
- **Requirement needs design** → Create a CIP (CIPs are just detailed requirements with implementation plans)
- **Requirement ready to implement** → Create a backlog task
- **Completed requirement** → Move to `requirements/archive/`
...
```
Created → Active → Under Review → Updated/Archived
```
...
├── README.md               # "How to think about requirements"
├── stakeholder-analysis.md # Thinking tool (VibeSafe guidance)
├── goal-decomposition.md   # Thinking tool (VibeSafe guidance)
├── constraint-mapping.md   # Thinking tool (VibeSafe guidance)
└── examples/
    ├── user-persona-example.md
    └── user-journey-example.md
```
...
requirements/
├── README.md
├── functional/             # Optional categorization
│   ├── REQ-001_user-auth.md
│   └── REQ-002_data-export.md
├── non-functional/         # Optional categorization
│   ├── REQ-010_performance.md
│   └── REQ-011_security.md
└── user-research/          # Optional - if project needs it
    ├── persona-admin.md
    └── persona-end-user.md
```
...
requirements/
├── REQ-001_user-login.md
├── REQ-002_data-export.md
└── REQ-003_email-notifications.md
...
│   └── REQ-002_data-export.md
├── non-functional/
│   ├── REQ-010_response-time.md
│   └── REQ-011_data-encryption.md
└── user-research/
    ├── persona-admin.md
    └── journey-onboarding.md
```
...
| **Requirements** | User Project | **WHAT** needs to be built | Starting new features | → Tenets |
| **CIPs** | User Project | **HOW** it will be built | Complex features | → Requirements |
| **Backlog** | User Project | **DOING** the work | Ready-to-implement | → CIPs |
...
Tenets (WHY - foundation)
    ↑
Requirements (WHAT) ──→ related_tenets
    ↑
CIPs (HOW) ──→ related_requirements
    ↑
Backlog (DO) ──→ related_cips
    ↓
Implementation

Scripts query DOWN to find dependencies:
- "Which requirements inform this tenet?" → Query requirements.related_tenets
- "Which CIPs implement this requirement?" → Query cips.related_requirements
- "Which tasks execute this CIP?" → Query backlog.related_cips
```
...
- **Main README**: Visual flow diagram showing WHAT → HOW → DO
...
→ Find requirements where related_tenets contains "simplicity-of-use"
→ Find CIPs where related_requirements contains those requirement IDs
→ Find backlog where related_cips contains those CIP IDs
...
Does it describe desired outcome?
    YES → Create/update requirement (WHAT)
    NO → Is it about design/approach?
        YES → Create/update CIP (HOW)
        NO → Is it ready-to-implement work?
            YES → Create backlog task (DO)
```
...
- If requirement reads like implementation → Refactor to focus on outcome
- If CIP doesn't reference requirements → Ask "What outcome does this achieve?"
- If understanding WHY is hard → Outcome isn't clear enough

## Implementation Plan

### Phase 0a: Create Validation Script (1-2 hours) ✅ **COMPLETED 2026-01-03**
...
- [x] **Output Format**:
  - [x] Color-coded: ✅ success, ⚠️  warnings, ❌ errors
...
**Deliverable**: ✅ `scripts/validate_vibesafe_structure.py` - our gold standard conformance checker

### Phase 0b: Integrate Validator into Install Script (1-2 hours) ✅ COMPLETED
...
**Status**: ✅ Completed 2026-01-03. Validator integrated into installation flow.  
**Deliverable**: Quality-assured installations with optional auto-fix

### Phase 0c: Integrate Validator into What's Next Script (1 hour) ✅ COMPLETED
...
**Status**: ✅ Completed 2026-01-03. Validator integrated into whats-next workflow.  
...
- [ ] Generate AI prompts for missing components:
  - [ ] Tenet extraction from codebase (scan code → suggest 3-5 tenets)
...
- [ ] Provide creation order: Tenets → Requirements → CIPs → Backlog
- [ ] Link requirements to tenets after extraction

**Deliverable**: Intelligent project bootstrapping guidance

### Phase 0: YAML Frontmatter and Naming Standardization ✅ COMPLETED (2026-01-03)
...
### Phase 1: Requirements Simplification ✅ COMPLETED (2026-01-03)

- [x] Create new `requirements/` directory structure (flat structure, optional categorization left to users)
- [x] Create simple `requirements/README.md` with YAML schema and optional categorization guide
- [x] **Document WHAT vs HOW distinction in README**:
  - [x] Explain: Requirements = WHAT should be true (outcomes, desired state)
  - [x] Explain: CIPs = HOW to achieve it (design, implementation approach)
  - [x] Explain: Backlog = DOING the work (execution tasks)
  - [x] Show flow: Requirement → CIP → Backlog Task → Implementation
  - [x] Provide examples of good requirements (outcome-focused) vs implementation details
  - [x] Add decision guide: "Am I describing WHAT or HOW?"
- [x] Create `templates/requirements/requirement_template.md` with YAML frontmatter
- [x] Migrate existing requirements from `ai-requirements/` to new format (removed related_backlog fields)
- [x] Update cursor rules to reflect new requirements approach:
  - [x] Change glob from `ai-requirements/**/*.md` to `requirements/**/*.md`
  - [x] Remove complex framework description (prompts, patterns in user projects)
  - [x] Replace with simple requirements format (YAML + description + acceptance criteria)
  - [x] **Add WHAT vs HOW guidance section**
  - [x] Add optional patterns reference
  - [x] Keep status sync table (still valid)
- [x] Archive old ai-requirements framework (added DEPRECATED.md, updated README)
- [x] **Update install-minimal.sh to handle migration (REQ-007, REQ-0008)**:
  - [x] Detect if ai-requirements/ exists in user project
  - [x] Notify users about deprecation (no forced migration)
  - [x] Update templates to install requirements/
  - [x] Preserve all user content during migration
  - [x] Clear messaging about what's being updated (system files) vs preserved (user content)

### Phase 2: Patterns → VibeSafe Guidance ✅ COMPLETED (2026-01-03)

- [x] Move patterns from `ai-requirements/patterns/` to `docs/patterns/` (VibeSafe guidance)
- [x] Create `docs/patterns/README.md` explaining patterns as thinking tools
- [x] Keep stakeholder-identification, goal-decomposition as guidance
- [x] Add examples showing how to apply patterns (VibeSafe dogfooding examples)
- [x] Update cursor rules to reference patterns as optional resources:
  - [x] Section already existed: "Need Help Writing Requirements?"
  - [x] Updated to link to VibeSafe patterns in `docs/patterns/` as reference material
  - [x] Clarified patterns are VibeSafe guidance, not user project structure
  - [x] Show patterns as thinking tools to consult when stuck
- [x] Patterns are VibeSafe-level documentation (not installed to user projects)
...
- [ ] Add cross-reference validation (requirements → CIPs → backlog)
...
- [ ] **Add WHAT vs HOW distinction to main documentation**:
  ...
  - [ ] Show the flow visually: WHAT → HOW → DO
...
- [ ] At least 3 example requirements in new format (✅ have 7!)
...
- **Python PEP Process**: Example of effective requirements → design flow
